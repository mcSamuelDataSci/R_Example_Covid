...
## Load neccesary packages, and read in data
...
## COVID-19 Case, Death, and Test Rates by R/E in Fresno County
```{r}

# Filter on Fresno county
covid_re_fresno <- covid_re_data %>%
  filter(county == "Fresno")

ggplot(data = covid_re_fresno, aes(x = demographic_set_category, y = metric_value_per_100k)) + 
  ...
  labs(title = "COVID-19 Rates by Race/Ethnicity in Fresno County", 
       x = "Race/Ethnicity", 
...
## Trend of COVID-19 Cases in Fresno County
```{r}

# Filter on Fresno county
covid_data_fresno <- covid_data %>%
  filter(county == "Fresno")

ggplot(data = covid_data_fresno, aes(x = year_month, y = Cases)) +
  geom_line(size = 1, colour = "darkblue") +
  labs(title = "Trend of COVID-19 Cases in Fresno County", 
       x = "Year-Month", 
...
## Trend of COVID-19 Deaths in Fresno County
```{r}
...
ggplot(data = covid_data_fresno, aes(x = year_month, y = Deaths)) +
  geom_line(size = 1, colour = "darkblue") +
  labs(title = "Trend of COVID-19 Deaths in Fresno County", 
       x = "Year-Month", 
